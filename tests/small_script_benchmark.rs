...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    Vm::bootstrap_without_init(vm.clone(), classes);
    c.bench_function("repl with cache", |b| b.iter(|| {
        Vm::repl_on_registered_class(vm.clone(), "_MainScript", Box::new(&vm_hook), vec![]).unwrap();
    }));
}
